...
SET "PORTFIFINE_REPOSITORY_ROOT=%~dp0"
SET "EXITCODE=0"
SET "NOPAUSE=0"
FOR %%A IN (%*) DO (
...
SET "PFM_A=PORTFIFINE_PAYLOAD_MARKER_"
SET "PFM_B=END_OF_BATCH_DO_NOT_EDIT"
SET "PAYLOAD_MARKER=%PFM_A%%PFM_B%"
FOR /F "usebackq delims=" %%T IN (`powershell.exe -NoProfile -Command "[System.IO.Path]::Combine([System.IO.Path]::GetTempPath(), 'Portfifine_Install_' + [Guid]::NewGuid().ToString('N') + '.ps1')"`) DO SET "TMPPS1=%%T"

FOR /F "tokens=1 delims=:" %%L IN ('FINDSTR /N /C:"%PAYLOAD_MARKER%" "%~f0"') DO SET "MARKER_LINE=%%L"
IF NOT DEFINED MARKER_LINE (
    ECHO ERRO: marcador de payload do instalador nao encontrado neste arquivo.
    SET "EXITCODE=1"
    GOTO :DONE
)

FOR /F "tokens=1* delims=:" %%L IN ('FINDSTR /N "^" "%~f0"') DO (
    IF %%L GTR %MARKER_LINE% >>"%TMPPS1%" ECHO(%%M
)

IF NOT EXIST "%TMPPS1%" (
    ECHO ERRO: falha ao extrair o payload do instalador.
    SET "EXITCODE=1"
    GOTO :DONE
)

powershell.exe -NoProfile -ExecutionPolicy RemoteSigned -File "%TMPPS1%" %*
SET "EXITCODE=%ERRORLEVEL%"

:DONE
IF EXIST "%TMPPS1%" DEL /F /Q "%TMPPS1%"
IF "%NOPAUSE%"=="0" (
    ECHO.
    PAUSE
)
EXIT /B %EXITCODE%
PORTFIFINE_PAYLOAD_MARKER_END_OF_BATCH_DO_NOT_EDIT
...
# Resolve repository root. The self-contained launcher sets
# PORTFIFINE_REPOSITORY_ROOT to the folder containing the .bat so the
# extracted payload (which lives in TEMP) can locate the repository. A
# user-supplied -RepositoryRoot always wins; otherwise the environment
# variable is preferred over $PSScriptRoot's parent.
if ([string]::IsNullOrWhiteSpace($RepositoryRoot)) {
    if (-not [string]::IsNullOrWhiteSpace($env:PORTFIFINE_REPOSITORY_ROOT)) {
        $RepositoryRoot = $env:PORTFIFINE_REPOSITORY_ROOT
    } else {
